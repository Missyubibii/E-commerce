...
@section('header', 'Chỉnh sửa quảng cáo')
...
<div class="bg-white p-8 rounded-lg shadow-md">
    <form action="{{ route('admin.banners.update', $banner) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="space-y-6">
            ...
            <div>
                <label for="image" class="block text-sm font-medium text-gray-700">Hình ảnh mới (để trống nếu không thay đổi)</label>
                ...
                @if($banner->image)
                    <div class="mt-4">
                        <p class="text-sm text-gray-500">Hình ảnh hiện tại:</p>
                        <img src="{{ asset('img/banner/' . $banner->image) }}" alt="{{ $banner->title }}" class="mt-2 w-48 h-24 object-cover rounded-md">
                    </div>
                @endif
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" name="position" id="position" value="{{ old('position', $banner->position) }}" required
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 ease-in-out">
                ...
                <div>
                    ...
                    <select name="is_active" id="is_active" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 transition duration-150 ease-in-out">
                        <option value="1" @selected(old('is_active', $banner->is_active))>Kích hoạt</option>
                        <option value="0" @selected(old('is_active', $banner->is_active) == 0)>Vô hiệu hóa</option>
                    </select>
